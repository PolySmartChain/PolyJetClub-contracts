...
contract PolyJetClubV2 is ERC721Enumerable, EIP712, Ownable, IPolyJetClub {
    ...
    event AddMap(uint256[] tokenIds, address[] tos);
    ...
    function hasMint(uint256[] memory tokenIds) internal view returns (bool) {
        for (uint256 x; x < tokenIds.length; x++) {
            if (_exists(tokenIds[x])) {
                return true;
            }
        }
        return false;
    ...
    function claim(uint256 tokenId) external payable {
        ...
        require(!hasMint(_asSingletonArray(tokenId)), "Token already minted");
    ...
    function claimBatch(uint256[] calldata tokenIds) external payable {
        ...
        require(!hasMint(tokenIds), "Token already minted");
        uint256 value1;
        uint256 nowFee = msg.value;
    ...
    function addMap(uint256[] calldata tokenIds, address[] calldata tos) external onlyOwner {
        require(tokenIds.length > 0 && tokenIds.length == tos.length, "Abnormal Error");
        require(!hasMint(tokenIds), "Token already minted");
        for (uint256 x; x < tokenIds.length; x++) {
            uint256 tokenId = tokenIds[x];
            date[tokenId] = block.timestamp;
            _safeMint(tos[x], tokenId);
        }
        emit AddMap(tokenIds, tos);
    }
